...
# # independent variable. We are using the same X value we are getting from the csv sheet. Now, if we use the same value
# Y
...
import pandas, matplotlib.pyplot as plot
from sklearn.linear_model import LinearRegression
...
plot.plot(X, regression.predict(X), color='red', linewidth=5)
plot.title("Film cost Vs Global Revenue")
plot.xlabel("Production Budget $")
plot.ylabel("Worldwide Gross $")
